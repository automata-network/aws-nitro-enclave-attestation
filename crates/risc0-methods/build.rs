...
fn main() {
    ...
    let elf_paths = if verifier_path.exists() && aggregator_path.exists() {
        println!("cargo::warning=Skipping build (pre-built ELFs exist in ./elf/)");
        (verifier_path, aggregator_path)
    } else {
        // Build
        ...
        for name in [RISC0_VERIFIER, RISC0_AGGREGATOR] {
            ...
            fs::copy(&src, &dst)
                .unwrap_or_else(|e| panic!("Failed to copy {:?} to {:?}: {}", src, dst, e));
        }

        (verifier_path, aggregator_path)
    };

    // Generate methods.rs with include_bytes! pointing to the ELF files
...
pub const RISC0_AGGREGATOR_ELF: &[u8] = include_bytes!({:?});"#,
    ...
    // Tell cargo to rerun if the ELF files change
    println!("cargo::rerun-if-changed={}", elf_paths.0.display());
    println!("cargo::rerun-if-changed={}", elf_paths.1.display());
}
